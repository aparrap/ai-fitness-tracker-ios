...
final class HealthKitMapper {
    ...
    func makeRequest(from startDate: Date, to endDate: Date = Date()) async throws -> AppleHealthImportRequest {
        ...
        let mappedWorkouts = try await mapWorkoutsWithOwnershipContext(healthWorkouts)
        ...
        return AppleHealthImportRequest(
            ...
            deletedWorkoutSourceRecordIds: []
        )
    }

    func makeIncrementalRequest(
        workouts: [HKWorkout],
        deletedWorkoutSourceRecordIds: [String],
        exportedAt: Date,
        syncId: String
    ) async throws -> AppleHealthImportRequest {
        AppleHealthImportRequest(
            syncId: syncId,
            exportedAt: exportedAt,
            device: deviceMetadata(),
            weights: [],
            workouts: try await mapWorkoutsWithOwnershipContext(workouts),
            deletedWorkoutSourceRecordIds: deletedWorkoutSourceRecordIds
        )
    }

    private func mapWorkoutsWithOwnershipContext(
        ...
        return try await mapWorkouts(healthWorkouts, ownershipContext: ownershipContext)
    }
    ...
    private func ownershipContext(for exportedWorkouts: [HKWorkout]) async throws -> [HKWorkout] {
        ...
        return try await reader.workoutsOverlapping(from: earliestStart, to: latestEnd)
    }
    ...
    private func mapWorkouts(
        ...
        for batchStart in stride(from: 0, to: healthWorkouts.count, by: maxConcurrentWorkoutMappings) {
            let batchEnd = min(batchStart + maxConcurrentWorkoutMappings, healthWorkouts.count)
            let batchResults = try await withThrowingTaskGroup(
                ...
                for index in batchStart..<batchEnd {
                    ...
                    let overlapping = overlappingWorkouts(for: workout, in: ownershipContext)
                    group.addTask { [self] in
                        (index, try await mapWorkout(workout, overlappingWorkouts: overlapping))
                    }
                }
        ...
        return indexedResults.sorted { $0.index < $1.index }.map { $0.workout }
    }

    private func overlappingWorkouts(for workout: HKWorkout, in workouts: [HKWorkout]) -> [HKWorkout] {
    ...
    private func mapWeight(_ sample: HKQuantitySample) -> AppleHealthWeight? {
        ...
        guard weightKg.isFinite && weightKg > 0 && weightKg <= 500 else { return nil }

        return AppleHealthWeight(
            sourceRecordId: sample.uuid.uuidString.lowercased(),
            measuredAt: sample.startDate,
            measuredOn: localDateString(sample.startDate, metadata: sample.metadata),
            weightKg: weightKg,
            heightCm: nil
        )
    }
    ...
    private func mapWorkout(
        ...
        let samples = try await detailedSamples(for: workout, overlappingWorkouts: overlappingWorkouts)
        let heartRates = samples.filter { $0.metric == "heart_rate" }.map(\.value)
        ...
        let validatedDurationSeconds = (0...(7 * 24 * 60 * 60)).contains(durationSeconds) ? durationSeconds : nil
        let rawDistanceM = workout.totalDistance?.doubleValue(for: .meter())
        let distanceM = rawDistanceM.flatMap { $0.isFinite && $0 >= 0 && $0 <= 500_000 ? $0 : nil }
        let rawActiveEnergyKcal = workout.totalEnergyBurned?.doubleValue(for: .kilocalorie())
        let activeEnergyKcal = rawActiveEnergyKcal.flatMap { $0.isFinite && $0 >= 0 && $0 <= 50_000 ? $0 : nil }
        ...
        return AppleHealthWorkout(
            ...
            startedOn: localDateString(workout.startDate, metadata: workout.metadata),
    ...
    private func detailedSamples(
        ...
        if workout.workoutActivityType == .running {
            definitions += [
                MetricDefinition(identifier: .runningSpeed, metric: "running_speed", unit: .meter().unitDivided(by: .second()), unitName: "m/s", aggregation: "instantaneous"),
                MetricDefinition(identifier: .distanceWalkingRunning, metric: "distance", unit: .meter(), unitName: "m", aggregation: "interval_delta"),
                MetricDefinition(identifier: .activeEnergyBurned, metric: "active_energy", unit: .kilocalorie(), unitName: "kcal", aggregation: "interval_delta"),
                MetricDefinition(identifier: .stepCount, metric: "step_count", unit: .count(), unitName: "count", aggregation: "interval_delta"),
                MetricDefinition(identifier: .runningPower, metric: "running_power", unit: .watt(), unitName: "W", aggregation: "instantaneous"),
                MetricDefinition(identifier: .runningStrideLength, metric: "running_stride_length", unit: .meter(), unitName: "m", aggregation: "instantaneous"),
                MetricDefinition(identifier: .runningVerticalOscillation, metric: "running_vertical_oscillation", unit: .meter(), unitName: "m", aggregation: "instantaneous"),
                MetricDefinition(identifier: .runningGroundContactTime, metric: "running_ground_contact_time", unit: HKUnit.secondUnit(with: .milli), unitName: "ms", aggregation: "instantaneous")
            ]
        }

        var result: [AppleHealthWorkoutSample] = []
        for batchStart in stride(from: 0, to: definitions.count, by: maxConcurrentMetricQueries) {
            let batchEnd = min(batchStart + maxConcurrentMetricQueries, definitions.count)
            ...
            let batchSamples = try await withThrowingTaskGroup(
                ...
                for definition in batch {
                    group.addTask { [reader] in
                        ...
                        return queryResult.samples.compactMap { sample in
                            ...
                            guard Self.isValidMetricValue(value, metric: definition.metric) else { return nil }
                ...
                for try await samples in group { mapped.append(contentsOf: samples) }
                return mapped
            }
        ...
        return result.sorted { left, right in
            if left.sampledAt == right.sampledAt {
                if left.metric == right.metric { return left.sourceRecordId < right.sourceRecordId }
    ...
    private static func isValidMetricValue(_ value: Double, metric: String) -> Bool {
        ...
        switch metric {
        case "heart_rate": return value > 0 && value <= 260
        case "running_speed": return value >= 0 && value <= 20
        case "distance": return value >= 0 && value <= 500_000
        case "active_energy": return value >= 0 && value <= 50_000
        case "step_count": return value >= 0 && value <= 1_000_000
        case "running_power": return value >= 0 && value <= 5_000
        case "running_stride_length": return value >= 0 && value <= 5
        case "running_vertical_oscillation": return value >= 0 && value <= 1
        case "running_ground_contact_time": return value >= 0 && value <= 5_000
    ...
    private func localDateString(_ date: Date, metadata: [String: Any]?) -> String {
    ...
    private func timeZone(from metadata: [String: Any]?) -> TimeZone? {
        if let timeZone = metadata?[HKMetadataKeyTimeZone] as? TimeZone { return timeZone }
        if let identifier = metadata?[HKMetadataKeyTimeZone] as? String { return TimeZone(identifier: identifier) }
        return nil
    }
